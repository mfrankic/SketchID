...
public class DrawingActivity extends AppCompatActivity {
    ...
    public static final int ITEM_ATTEMPTS = 4;
    ...
    private void initializeDrawingActivity() {
        ...
        nextImageButton.setOnClickListener(v -> {
            if (drawingDataList.isEmpty()) {
                Toast.makeText(this, "Please draw something before moving to the next image.", Toast.LENGTH_SHORT).show();
                return;
            }
    ...
    private void reInitializeDrawingView() {
        ...
        drawingView.setOnStrokeListener((x, y, timestamp, action) -> {
            ...
            int currentItemId = items.get(currentItemIndex).id;
            Item.Type currentItemType = items.get(currentItemIndex).type;

            int imageID = (currentItemType == Item.Type.IMAGE) ? currentItemId : -1;

            if (currentItemId != -1 && drawingDataList.isEmpty()) {
                startTimestamp = timestamp;
                drawingDataList.add(new DrawingData(time, x, y, "START", selectedUserId, imageID, items.get(currentItemIndex).type, currentItemAttempt));
            }

            drawingDataList.add(new DrawingData(time, x, y, action, selectedUserId, imageID, items.get(currentItemIndex).type, currentItemAttempt));
        });
    }
    ...
    private void loadItems() {
        ...
        for (Image image : images) {
            items.add(new Item(Item.Type.IMAGE, image.id, image.name, image.source));
        }
    ...
    private void loadCurrentItem() {
        ...
        switch (currentItem.type) {
            case IMAGE:
                referenceImage.setImageResource(currentItem.resource);
